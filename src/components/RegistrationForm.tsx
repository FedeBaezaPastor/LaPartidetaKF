    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-emerald-50">
      <div className="max-w-lg mx-auto px-4 py-6">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 md:p-8">
          <h1 className="text-2xl font-bold text-gray-900 mb-1">Crear cuenta {planType === 'player' ? 'Player' : 'Team'}</h1>
          <p className="text-sm text-gray-500 mb-6">Configura tu perfil de jugador</p>
            ...
            <div className="flex flex-wrap gap-3">
              {AVATAR_PRESETS.map((url, i) => (
                <button
                  ...
                  className={`w-14 h-14 rounded-full overflow-hidden border-2 transition-all ${avatarUrl === url ? 'border-emerald-600 ring-2 ring-emerald-200' : 'border-gray-200'}`}
                >
          ...
          <div className="mb-4">
            ...
            <div className="relative">
              <input
                ...
                className="w-full px-4 py-2.5 border rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-transparent pr-10"
                style={{ borderColor: nickStatus === 'available' ? '#059669' : nickStatus === 'taken' ? '#ef4444' : '#d1d5db' }}
              />
              <div className="absolute right-3 top-1/2 -translate-y-1/2">
                {nickStatus === 'checking' && <div className="w-4 h-4 border-2 border-gray-300 border-t-emerald-600 rounded-full animate-spin" />}
                {nickStatus === 'available' && <Check size={18} className="text-emerald-600" />}
                {nickStatus === 'taken' && <X size={18} className="text-red-500" />}
              </div>
            </div>
            {nickStatus === 'available' && <p className="text-xs text-emerald-600 mt-1">Nick disponible</p>}
            {nickStatus === 'taken' && <p className="text-xs text-red-500 mt-1">Este Nick ya esta cogido</p>}
            {nickStatus === 'idle' && nick.length > 0 && <p className="text-xs text-gray-400 mt-1">Minimo 3 caracteres</p>}
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4 mb-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Contrasena <span className="text-gray-400 text-xs">(opcional)</span></label>
              <div className="relative">
                <input
                  ...
                  className="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-transparent pr-10"
                />
                <button
          ...
          <div className="grid md:grid-cols-2 gap-4 mb-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Handicap exacto</label>
              <input
                type="number"
                step="0.1"
                value={handicap}
          ...
          {/* Default tee */}
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 mb-2">Barras por defecto</label>
            <div className="flex gap-2">
              {TEE_OPTIONS.map(t => (
                <button
                  key={t}
                  onClick={() => setDefaultTee(t)}
                  className={`px-4 py-2 rounded-lg text-sm font-medium capitalize transition-all ${defaultTee === t ? 'bg-emerald-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                >
                  {t}
                </button>
              ))}
          ...
          <div className="space-y-3 mb-6">
            <label className="flex items-start gap-2 cursor-pointer">
              ...
              <span className="text-sm text-gray-600">Confirmo que tengo al menos 14 anos</span>
            </label>
            <label className="flex items-start gap-2 cursor-pointer">
              ...
              <span className="text-sm text-gray-600">Acepto los <a href="#" className="text-emerald-600 underline">terminos</a> y la <a href="#" className="text-emerald-600 underline">politica de privacidad</a></span>
            </label>
          </div>
