...
const Tooltip: React.FC<TooltipProps> = ({ text }) => {
  ...
  return (
    <div className="relative inline-block">
      ...
      {show && (
        <div className="absolute z-50 left-1/2 -translate-x-1/2 top-6 w-56 bg-slate-800 text-white text-xs p-2.5 rounded-lg shadow-lg">
          {text}
        </div>
      )}
    </div>
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-emerald-50">
      <div className="max-w-lg mx-auto px-4 py-6">
        ...
        <div className="flex items-center gap-2 mb-6">
          {[1, 2, 3].map(s => (
            <div key={s} className={`h-2 flex-1 rounded-full transition-all ${s <= step ? 'bg-emerald-600' : 'bg-gray-200'}`} />
          ))}
        </div>
        ...
        {step === 1 && (
          <div className="bg-white rounded-2xl shadow-xl p-6">
            <h2 className="text-xl font-bold text-gray-900 mb-1">Crear Team</h2>
            <p className="text-sm text-gray-500 mb-5">Configura tu grupo de juego</p>

            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-1">Nombre del grupo <span className="text-red-500">*</span></label>
              <input
                type="text"
                value={groupName}
            ...
            <div className="mb-6">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="px-4 py-2.5 bg-gray-100 hover:bg-gray-200 rounded-xl text-sm font-medium text-gray-700"
                >
                  Generar
                </button>
              </div>
        ...
        {step === 2 && (
          <div className="bg-white rounded-2xl shadow-xl p-6">
            <h2 className="text-xl font-bold text-gray-900 mb-5">Configuracion del Team</h2>
              ...
              <div className="border border-gray-200 rounded-xl p-4">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${addPlayersNow ? 'bg-emerald-600 text-white' : 'bg-gray-100 text-gray-600'}`}
                  >
                    Ahora
                  </button>
                  <button
              ...
              <div className="border border-gray-200 rounded-xl p-4">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${assignAdmin ? 'bg-emerald-600 text-white' : 'bg-gray-100 text-gray-600'}`}
                  >
                    Si
                  </button>
                  <button
              ...
              <div className="border border-gray-200 rounded-xl p-4">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${hoyo19 ? 'bg-emerald-600 text-white' : 'bg-gray-100 text-gray-600'}`}
                  >
                    Si, incluir
                  </button>
                  <button
        ...
        {step === 3 && (
          <div className="bg-white rounded-2xl shadow-xl p-6">
            <h2 className="text-xl font-bold text-gray-900 mb-1">Invitar jugadores</h2>
            <p className="text-sm text-gray-500 mb-5">Busca por Nick y envia invitaciones</p>
                ...
                {searchResults.map(p => (
                  ...
                  >
                    ...
                    <div className="flex-1 text-left">
                      <p className="font-medium text-gray-800 text-sm">{p.nick}</p>
                      {p.display_name && <p className="text-xs text-gray-500">{p.display_name}</p>}
                    </div>
            ...
            {selectedPlayers.length > 0 && (
              <div className="mb-4">
                <p className="text-sm font-medium text-gray-700 mb-2">Jugadores seleccionados:</p>
                <div className="space-y-2">
                  {selectedPlayers.map(p => (
                    <div key={p.user_id} className="flex items-center gap-3 bg-emerald-50 rounded-lg px-3 py-2">
                      {p.avatar_url ? (
                        <img src={p.avatar_url} alt="" className="w-7 h-7 rounded-full" />
                      ) : (
                        <div className="w-7 h-7 bg-emerald-200 rounded-full" />
                      )}
                      <span className="flex-1 text-sm font-medium text-gray-800">{p.nick}</span>
        ...
        {step === 4 && (
          <div className="bg-white rounded-2xl shadow-xl p-8 text-center">
            ...
            <p className="text-gray-500 mb-2">{groupName}</p>
            ...
            <div className="bg-gray-50 rounded-xl px-4 py-3 mb-6">
              <p className="text-xs text-gray-500 mb-1">Codigo de acceso</p>
              <p className="text-2xl font-mono font-bold text-emerald-600 tracking-wider">{groupCode}</p>
            </div>
            <button
