/*
  # Fix Guti player name in archived rounds

  1. Purpose
    - Update "José Luis - Guti" to "Gutti" in all archived rounds
    - Update player_id to point to current player record
...
-- Update all references to José Luis - Guti in group 355d0af9-0a96-4d6d-ab5a-ef1c2b203c76
...
SET
  ...
  final_ranking = (
    SELECT jsonb_agg(
      CASE 
        WHEN elem->>'player_name' = 'José Luis - Guti'
        THEN jsonb_set(
          jsonb_set(elem, '{player_name}', to_jsonb('Gutti'::text)),
          '{player_id}', 
          to_jsonb('803493eb-bafa-486f-afc0-b209a3558c12'::text)
        )
        ELSE elem
      END
    )
...
WHERE 
  ...
  AND EXISTS (
    ...
    WHERE elem->>'player_name' = 'José Luis - Guti'
  );