...
-- Nick lookup is exposed through restricted RPCs so private profile fields
-- (age, postal code, country, etc.) never become publicly selectable.
...
STABLE
AS $$
  SELECT p.id, p.user_id, p.nick, p.display_name, p.avatar_url, p.exact_handicap
  FROM user_profiles p
  WHERE auth.uid() IS NOT NULL
    AND length(trim(search_term)) >= 2
    AND p.nick ILIKE '%' || trim(search_term) || '%'
  ORDER BY p.nick
  LIMIT 10;
$$;

CREATE OR REPLACE FUNCTION is_nick_available(candidate text)
RETURNS boolean
LANGUAGE sql
SECURITY DEFINER
SET search_path = public
STABLE
AS $$
  SELECT length(trim(candidate)) >= 2
    AND NOT EXISTS (
      SELECT 1 FROM user_profiles p WHERE lower(p.nick) = lower(trim(candidate))
    );
$$;

REVOKE ALL ON FUNCTION search_user_profiles(text) FROM PUBLIC;
REVOKE ALL ON FUNCTION is_nick_available(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION search_user_profiles(text) TO authenticated;
GRANT EXECUTE ON FUNCTION is_nick_available(text) TO anon;
GRANT EXECUTE ON FUNCTION is_nick_available(text) TO authenticated;
...
-- SECURITY DEFINER helpers prevent recursive RLS evaluation when policies
-- need to inspect memberships in this same table.
...
REVOKE ALL ON FUNCTION is_group_admin(uuid) FROM PUBLIC;
  ...
  TO authenticated WITH CHECK (
    ...
    AND (
      ...
      OR EXISTS (
        ...
        WHERE g.id = group_id AND g.user_auth_id = auth.uid()
      )
    )
  );
