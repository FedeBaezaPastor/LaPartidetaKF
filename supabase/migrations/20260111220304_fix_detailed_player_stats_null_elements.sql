/*
  # Fix Detailed Player Statistics to Handle Null Elements

  1. Changes
    - Update get_detailed_player_statistics to filter out null elements from hole_scores
    - Ensure that only valid hole score records are counted

  2. Impact
    - Player individual statistics will now correctly show distribution and analysis
    - Fixes missing "Distribución de Resultados" and "Análisis de juego" sections
*/
...
BEGIN
  ...
  -- Count total holes played (filter out null elements)
  ...
  -- Get hole results aggregation (calculate from net_strokes - par, filter out null elements)
  SELECT jsonb_build_object(
