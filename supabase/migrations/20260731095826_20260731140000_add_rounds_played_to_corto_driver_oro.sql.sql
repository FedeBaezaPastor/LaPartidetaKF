...
RETURNS TABLE(player_name text, no_paso_rojas_count numeric, total_rounds bigint)
LANGUAGE plpgsql
AS $$
BEGIN
  RETURN QUERY
  SELECT
    ...
    SUM(COALESCE((stat->>'no_paso_rojas_count')::bigint, 0)) AS no_paso_rojas_count,
    COUNT(DISTINCT ar.id) AS total_rounds
