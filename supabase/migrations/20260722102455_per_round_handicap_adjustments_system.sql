...
--
-- Redesigned: each archived round is processed independently.
-- - A round's final_ranking determines each player's position
-- - Top half: -1 to exact_handicap (if >0), -1 to playing_handicap (min 0)
-- - Bottom half: +1 to exact_handicap (if <12), +1 to playing_handicap (if <=11)
-- - Middle (odd count): no change
-- - Once processed, the round is marked handicap_adjusted=true
-- - If a player plays two rounds in a day, each adjusts independently
-- ============================================================================
...
-- Step 3: Add FK (SET NULL on delete, matching handicap_history behavior)
DO $$ BEGIN
  IF NOT EXISTS (
...
-- (their handicaps were already set by the old system)
...
BEGIN
  ...
  -- Skip if round not found or already adjusted
  IF NOT FOUND OR v_round.handicap_adjusted THEN
    RETURN;
  END IF;

  -- Count players in this round's final_ranking
  ...
  -- Process each player based on their position in THIS round
  FOR v_player IN
    SELECT 
  ...
  LOOP
    ...
    -- Top half: -1 (exact if >0, playing min 0)
    ...
    -- Bottom half: +1 (exact if <12, playing if <=11)
    ELSE
    ...
    -- Apply playing_handicap cap: only +1 if <= 11
    ...
    IF v_adjustment != 0 OR v_playing_adj != 0 THEN
      ...
      -- Record the adjustment linked to this round
  ...
  -- Mark round as handicap adjusted
